...
import { getWalletPortfolio, getWalletBackendState } from "@/backend/service";
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const [portfolio, context] = await Promise.all([
      getWalletPortfolio(parsed.data.wallet),
      Promise.resolve(getWalletBackendState(parsed.data.wallet))
    ]);
    return NextResponse.json({ portfolio, context });
  } catch (error) {
